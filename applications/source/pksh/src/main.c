#include <stdint.h>
#include <stdbool.h>
#include <stdio.h>

#include <string.h>

#include <unistd.h>
#include <fcntl.h>
#include <errno.h>

#define BUFFER_SIZE (10)

void run_line(char * line);

int main(uint64_t argc, const char ** argv) {
    int64_t line_size = 0;
    static char line_buf[512];

    if (argc == 1) {
        while (true) {
            printf("> ");

            while (true) {
                char c;
                read(STDIN_FILENO, &c, 1);

                if (c == '\n') {
                    line_buf[line_size] = '\0';
                    break;
                }
                else {
                    line_buf[line_size] = c;

                    line_size++;
                }
            }

            run_line(line_buf);

            line_size = 0;
        }
    }
    else if (argc == 2) {
        fd_t in_fd = open(argv[1], O_RDONLY);

        while (true) {
            while (true) {
                int64_t amount_read = read(in_fd, line_buf + line_size, 1);

                if (amount_read == 0 || line_buf[line_size] == '\n') {
                    line_buf[line_size] = '\0';
                    break;
                }
                else line_size++;
            }

            if (line_size == 0) break;
            run_line(line_buf);
            line_size = 0;
        }

        close(in_fd);
    }
    else {
        printf("Invalid arguments\n");
    }

    return 0;
}

void run(char ** argv, uint64_t argc, fd_t in, fd_t out, bool background) {
    uint64_t pipe_pos = 0;
    for (uint64_t i = 0; i < argc; i++) {
        if (strcmp(argv[i], "|") == 0) {
            pipe_pos = i;

            fd_t pipe_fds[2];
            pipe(pipe_fds, 0);

            run(argv, pipe_pos, in, pipe_fds[0], false);
            run(argv + pipe_pos + 1, argc - pipe_pos - 1, pipe_fds[1], out, false);

            return;
        }
        else if (strcmp(argv[i], ">") == 0) {
            pipe_pos = i;

            fd_t out_file = open(argv[pipe_pos + 1], O_WRONLY | O_CREAT);

            run(argv, pipe_pos, in, out_file, false);

            return;
        }
        else if (strcmp(argv[i], "&") == 0) {
            pipe_pos = i;

            run(argv, pipe_pos, in, out, true);

            return;
        }
    }

    if (strcmp(argv[0], "cd") == 0) {
        if (chdir(argv[1]) != ERROR_OK) {
            printf("cd: Unable to change directory\n");
        }
    }
    else if (strcmp(argv[0], "exit") == 0) {
        exit(0);
    }
    else {
        pid_t fork_result = fork();

        if (fork_result == 0) { // child
            char path[5 + strlen(argv[0]) + 1];
            strcpy(path, "/bin/");
            strcpy(path + 5, argv[0]);

            dup(STDOUT_FILENO, out);
            dup(STDIN_FILENO, in);

            error_number_t exec_result = exec(path, (const char **) argv, argc);

            if (exec_result != 0) {
                printf("Unable to execute \"%s\": %s\n", argv[0], strerrordesc_np(-exec_result));

                exit(1);
            }
        }
        else { // parent
            if (!background) wait();
        }
    }
}

void run_line(char * line) {
    if (line[0] == '\0') return;

    int argc = 1;
    char * argv[100] = { line };

    for (uint64_t i = 0; line[i] != '\0'; i++) {
        if (line[i] == ' ') {
            line[i] = '\0';

            argv[argc++] = &line[i + 1];
        }
    }

    run(argv, argc, STDIN_FILENO, STDOUT_FILENO, false);
}
