...
primary_memory_region_t paging_alloc_region;

void paging_init() {
    paging_alloc_region = get_primary_memory_region();
    paging_alloc_region.base += KERNEL_SIZE;
    paging_alloc_region.length -= KERNEL_SIZE;
    ...
    memset(paging_table_space_pt,  0, sizeof(pt64_t));
    ...
    kernel_physical_address = (uint64_t) PT64_GET_ADDRESS((*old_pt)[0]);

    pml4t64_entry_t * pml4t_entry = pml4t64_map_address(
        &paging_kernel_pml4t,
        (void *) paging_kernel_virtual_to_physical(paging_kernel_pdpt),
        KERNEL_START
    );
    ...
    pdpt64_entry_t * pdpt_entry  = pdpt64_map_address(
        &paging_kernel_pdpt,
        (void *) paging_kernel_virtual_to_physical(paging_kernel_pdt),
        KERNEL_START
    );
    ...
    pdt64_entry_t * pdt_entry = pdt64_map_address(
        &paging_kernel_pdt,
        (void *) paging_kernel_virtual_to_physical(paging_kernel_pt),
        KERNEL_START
    );
    pdt_entry->present = 1;
